...
public class LZMAEncoder
{
    ...
    class LiteralEncoder
    {
        class Encoder2
        {
            ...
            public void init()
            {
                lzma.Encoder.initBitModels(m_Encoders);
            }


            public void encode(lzma.Encoder rangeEncoder, byte symbol) throws IOException
            {
                int context = 1;
                for (int i = 7; i >= 0; i--)
                {
            ...
            public void encodeMatched(lzma.Encoder rangeEncoder, byte matchByte, byte symbol) throws IOException
            {
                int context = 1;
                boolean same = true;
                for (int i = 7; i >= 0; i--)
            ...
            public int getPrice(boolean matchMode, byte matchByte, byte symbol)
            {
                ...
                if (matchMode)
                {
                    for (; i >= 0; i--)
                    {
                        int matchBit = (matchByte >> i) & 1;
                        int bit = (symbol >> i) & 1;
                        price += lzma.Encoder.getPrice(m_Encoders[((1 + matchBit) << 8) + context], bit);
                        context = (context << 1) | bit;
                        if (matchBit != bit)
                        {
                            i--;
                ...
                {
                    int bit = (symbol >> i) & 1;
                    price += lzma.Encoder.getPrice(m_Encoders[context], bit);
                    context = (context << 1) | bit;
                }
                return price;
            }
    ...
    class LenEncoder
    {
        ...
        {
            lzma.Encoder.initBitModels(_choice);
        ...
        public void encode(lzma.Encoder rangeEncoder, int symbol, int posState) throws IOException
        {
        ...
        {
            int a0 = lzma.Encoder.getPrice0(_choice[0]);
            int a1 = lzma.Encoder.getPrice1(_choice[0]);
            int b0 = a1 + lzma.Encoder.getPrice0(_choice[1]);
            int b1 = a1 + lzma.Encoder.getPrice1(_choice[1]);
            int i;
    ...
    lzma.BinTree _matchFinder = null;
    lzma.Encoder _rangeEncoder = new lzma.Encoder();
    ...
    void create()
    {
        if (_matchFinder == null)
        {
            ...
            lzma.BinTree bt = new lzma.BinTree(numHashBytes);
            _matchFinder = bt;
        }
    ...
    void init()
    {
        ...
        lzma.Encoder.initBitModels(_isMatch);
        lzma.Encoder.initBitModels(_isRep0Long);
        lzma.Encoder.initBitModels(_isRep);
        lzma.Encoder.initBitModels(_isRepG0);
        lzma.Encoder.initBitModels(_isRepG1);
        lzma.Encoder.initBitModels(_isRepG2);
        lzma.Encoder.initBitModels(_posEncoders);
    ...
    {
        return lzma.Encoder.getPrice0(_isRepG0[state]) +
                lzma.Encoder.getPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
    }
    ...
    int getPureRepPrice(int repIndex, int state, int posState)
    {
        ...
        if (repIndex == 0)
        {
            price = lzma.Encoder.getPrice0(_isRepG0[state]);
            price += lzma.Encoder.getPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
        }
        else
        {
            price = lzma.Encoder.getPrice1(_isRepG0[state]);
            if (repIndex == 1)
            {
                price += lzma.Encoder.getPrice0(_isRepG1[state]);
            }
            else
            {
                price += lzma.Encoder.getPrice1(_isRepG1[state]);
                price += lzma.Encoder.getPrice(_isRepG2[state], repIndex - 2);
            }
        }
        return price;
    }
    ...
    int getOptimum(int position) throws IOException
    {
        ...
        _optimum[1].Price = lzma.Encoder.getPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) +
                _literalEncoder.getSubCoder(position, _previousByte).getPrice(!Base.stateIsCharState(_state), matchByte, currentByte);
        _optimum[1].makeAsChar();

        int matchPrice = lzma.Encoder.getPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
        int repMatchPrice = matchPrice + lzma.Encoder.getPrice1(_isRep[_state]);
        ...
        int normalMatchPrice = matchPrice + lzma.Encoder.getPrice0(_isRep[_state]);
        ...
        while (true)
        {
            ...
            int curAnd1Price = curPrice +
                    lzma.Encoder.getPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
            ...
            matchPrice = curPrice + lzma.Encoder.getPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
            repMatchPrice = matchPrice + lzma.Encoder.getPrice1(_isRep[state]);
            ...
            if (!nextIsChar && matchByte != currentByte)
            {
                ...
                if (lenTest2 >= 2)
                {
                    ...
                    int nextRepMatchPrice = curAnd1Price +
                            lzma.Encoder.getPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
                            lzma.Encoder.getPrice1(_isRep[state2]);
                    {
                    ...
                    if (lenTest2 >= 2)
                    {
                        ...
                        int curAndLenCharPrice =
                                repMatchPrice + getRepPrice(repIndex, lenTest, state, posState) +
                                        lzma.Encoder.getPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
                        ...
                        int nextMatchPrice = curAndLenCharPrice + lzma.Encoder.getPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
                        int nextRepMatchPrice = nextMatchPrice + lzma.Encoder.getPrice1(_isRep[state2]);
            ...
            if (newLen >= startLen)
            {
                normalMatchPrice = matchPrice + lzma.Encoder.getPrice0(_isRep[state]);
    ...
    void fillDistancesPrices()
    {
        ...
        for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++)
        {
            ...
            {
                _posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << lzma.Encoder.kNumBitPriceShiftBits);
            }
